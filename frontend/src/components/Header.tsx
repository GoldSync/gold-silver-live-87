import { Moon, Sun, Activity, RefreshCw } from 'lucide-react';
import logo from '@/assets/goldsync.png';
import { PriceData } from '@/hooks/useGoldPrices';
import { AnimatedNumber } from './AnimatedNumber';
import { useState, useEffect } from 'react';
import { Button } from '@/components/ui/button';

interface HeaderProps {
  isDark: boolean;
  onToggleTheme: () => void;
  spot: PriceData | null;
  previousSpot: PriceData | null;
  lastUpdated: Date | null;
  onRefresh: () => void;
  isRefreshing: boolean;
  lastGoldTrend?: 'up' | 'down' | 'flat';
  lastSilverTrend?: 'up' | 'down' | 'flat';
  lastGoldDiff?: number;
  lastSilverDiff?: number;
}

function SpotBadge({
  label,
  price,
  trend = 'flat',
  diff = 0,
  bid,
  ask,
  decimals = 3
}: {
  label: string;
  price: number;
  trend?: 'up' | 'down' | 'flat';
  diff?: number;
  bid?: number;
  ask?: number;
  decimals?: number;
}) {
  const isUp = trend === 'up';
  const isDown = trend === 'down';

  return (
    <div className="flex flex-col items-center gap-1">
      <span className="text-sm font-sans uppercase tracking-widest text-muted-foreground">{label}</span>
      <div className="flex items-center gap-1.5">
        <AnimatedNumber
          value={price}
          prefix="$"
          decimals={decimals}
          className="text-xl md:text-2xl font-semibold font-sans tabular-nums text-foreground"
        />
        {(isUp || isDown) && (
          <span className={`text-xs font-sans font-medium ${isUp ? 'text-success' : 'text-destructive'}`}>
            {isUp ? '▲' : '▼'} {Math.abs(diff).toFixed(2)}
          </span>
        )}
      </div>
      {/* Bid Row */}
      <div className="flex items-center justify-center gap-3 text-xs font-sans text-muted-foreground mt-0.5">
        {bid && (
          <div className="flex gap-1">
            <span className="opacity-70">Bid:</span>
            <AnimatedNumber value={bid} prefix="$" decimals={decimals} className="font-medium tabular-nums text-foreground/80" />
          </div>
        )}
      </div>
    </div>
  );
}

function LiveClock() {
  const [now, setNow] = useState(new Date());

  useEffect(() => {
    const timer = setInterval(() => setNow(new Date()), 1000);
    return () => clearInterval(timer);
  }, []);

  const day = now.toLocaleDateString('en-US', { weekday: 'long' });
  const date = now.toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' });
  const time = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

  return (
    <div className="flex items-center gap-3 text-sm font-sans text-muted-foreground">
      <span>{day}</span>
      <span className="text-border">·</span>
      <span>{date}</span>
      <span className="text-border">·</span>
      <span className="tabular-nums text-primary font-medium">{time}</span>
    </div>
  );
}

export function Header({
  isDark,
  onToggleTheme,
  spot,
  previousSpot,
  onRefresh,
  isRefreshing,
  lastGoldTrend = 'flat',
  lastSilverTrend = 'flat',
  lastGoldDiff = 0,
  lastSilverDiff = 0
}: HeaderProps) {
  return (
    <header className="border-b border-border/50 backdrop-blur-sm bg-background/80 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        {/* Top row: theme toggle */}
        <div className="flex justify-end gap-2 mb-3">
          <Button
            variant="ghost"
            size="icon"
            onClick={onRefresh}
            disabled={isRefreshing}
            className="rounded-full hover:bg-secondary text-muted-foreground hover:text-foreground"
            title="Refresh Prices"
          >
            <RefreshCw className={`w-5 h-5 ${isRefreshing ? 'animate-spin' : ''}`} />
          </Button>

          <button
            onClick={onToggleTheme}
            className="p-2 rounded-full hover:bg-secondary transition-colors"
            aria-label="Toggle theme"
          >
            {isDark ? (
              <Sun className="w-5 h-5 text-primary" />
            ) : (
              <Moon className="w-5 h-5 text-muted-foreground" />
            )}
          </button>
        </div>

        {/* Brand name centered */}
        <div className="text-center mb-2 flex flex-col items-center gap-4">
          <img src={logo} alt="GoldSync Logo" className="h-40 w-auto" />
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-foreground tracking-tight uppercase">
            GoldSync
          </h1>
        </div>

        {/* Brand name centered */}

        {/* Date/time centered */}
        <div className="flex justify-center mb-5">
          <LiveClock />
        </div>

        {/* Live indicator */}
        <div className="flex justify-center mb-4">
          <div className="flex items-center gap-2 text-xs font-sans text-muted-foreground">
            {isRefreshing ? (
              <span className="text-primary animate-pulse">Updating...</span>
            ) : (
              <>
                <Activity className="w-3 h-3 text-success pulse-live" />
                <span>Live Prices</span>
              </>
            )}
          </div>
        </div>

        {/* Spot prices */}
        {spot && (
          <div className="flex items-center justify-center gap-8 sm:gap-12 animate-fade-in-up">
            <SpotBadge
              label="Gold / oz"
              price={spot.goldSpotUSD}
              bid={spot.goldBid}
              ask={spot.goldAsk}
              decimals={2} // Gold Header: 2 decimals
              trend={lastGoldTrend}
              diff={lastGoldDiff}
            />
            <div className="w-px h-10 bg-border" />
            <SpotBadge
              label="Silver / oz"
              price={spot.silverSpotUSD}
              bid={spot.silverBid}
              ask={spot.silverAsk}
              decimals={3} // Silver Header: 3 decimals
              trend={lastSilverTrend}
              diff={lastSilverDiff}
            />
          </div>
        )}
      </div>
    </header>
  );
}

